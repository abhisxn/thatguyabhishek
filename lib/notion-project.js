...
async function _getProjectPageData(pageId) {
  ...
  // Safety pass: fetch children for any callout or column_list not covered by
  // buildChildrenMap. Notion can return has_children:false for these blocks even
  // when they do have children, causing buildChildrenMap to skip them.
  ...
  // column_list safety pass — must run after callout pass so allProjectBlocks is complete.
  // Also recurse into the fetched columns so their children are populated too.
  await Promise.all(
    allProjectBlocks
      .filter((b) => b.type === 'column_list' && !childrenMap[b.id])
      .map(async (b) => {
        const cols = await fetchChildren(b.id);
        childrenMap[b.id] = cols;
        await buildChildrenMap(cols, childrenMap);
      })
  );
...
export function getProjectPageData(pageId) {
  // Fast path: pre-built static cache populated by notion-sync.js at build time.
  // Skipped in development so local edits in Notion appear immediately.
  if (process.env.NODE_ENV !== 'development') {
    const cached = pagesJson[pageId];
    if (cached) return Promise.resolve(cached);
  }

  // Fallback: live Notion API with ISR caching (for new projects not in build cache)
