...
import { getCalloutType } from '../app/components/ui/card-utils.js';
...
/* ── Writing Articles ────────────────────────────────────────────── */
const WRITING_DS_ID = '333848f1-3d34-8067-9369-000b755c73ef';
...
function mapArticlePage(page) {
  ...
  return {
    ...
    emoji: page.icon?.emoji || '✍️',
    title,
    desc: page.properties.Hook?.rich_text?.map((t) => t.plain_text).join('') || '',
    topic: page.properties['Topic Pillar']?.multi_select?.map((t) => t.name) || [],
    stage: page.properties['Writing Stage']?.select?.name || '',
    lastEdited: page.last_edited_time,
    href: `/writing/${slugifyArticle(title)}`,
  };
}

async function _getWritingArticles() {
  const res = await withRetry(() =>
    limit(() => notion.dataSources.query({
      data_source_id: WRITING_DS_ID,
      sorts: [{ property: 'Sort', direction: 'ascending' }],
    }))
  );
  return res.results.map(mapArticlePage);
}

export const getWritingArticles = unstable_cache(
  _getWritingArticles,
  ['writing-articles'],
  { revalidate: 3600, tags: ['notion'] }
);
...
async function _getArticleBySlug(slug) {
  const articles = await _getWritingArticles();
...
async function _getArticleBlocks(pageId) {
  const results = [];
  let cursor;
  do {
    const res = await withRetry(() =>
  ...
  // Build childrenMap for blocks that have children
  const childrenMap = {};
  await Promise.all(
    results
      .filter((b) => b.has_children)
      .map(async (b) => {
        const kids = await withRetry(() =>
          limit(() => notion.blocks.children.list({ block_id: b.id, page_size: 100 }))
        );
        childrenMap[b.id] = kids.results;
      })
  );

  return { blocks: results, childrenMap };
}

export const getArticleBlocks = unstable_cache(
  _getArticleBlocks,
  ['article-blocks'],
  { revalidate: 3600, tags: ['notion'] }
);
